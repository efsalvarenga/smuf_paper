...
\begin{figure}
  \centering
  \includegraphics[scale=0.5]{2018-01-03_randomgrouping_frontier}
...
% MAKE THE LINK BETWEEN SECTIONS, ESPECIALLY WITH DEMAND AGGREGATION
...
\begin{figure}
  \centering
  \includegraphics[scale=0.5]{2017-10-13_compare_demands_bw2}
  \caption{Example of electricity demand pattern for four different customers during a period of four weeks}
  \label{fig:4weeks}
\end{figure}

Datasets containing 12 weeks were used for model fitting and forecast generation for up to the next 72 hours.
We had access to a a large high frequency dataset.
...
\label{sec:load_fcst}
%===========================================
Generally, medium- and long-term forecasts must take into account:
\begin{enumerate}
  \item historical load and weather data;
  \item number of customers and their characteristics including demographics
  \item economics data, such as employment, industrial output, etc.
\end{enumerate}

These longer term forecasts are used to predict loads as distant as twenty years ahead, so that expansion planning can be facilitated \citep{feinberg2005load}.
Large generation and infrastructure projects depend on this type of forecasting for supporting decision making \citep{soliman1997application}.
...
However, for lead times up to 1-day-ahead, there is little difference in performance if weather forecast data is added to models.
Non-weather models accuracy worsen as lead time increases, where more complex models dominates accuracy \citep{TAYLOR200357}.
...
While the decision making process in the utility industry rely on expected values, or point forecast, the increase in market competition and renewable integration requires a probabilistic approach for planning and operation of energy systems \citep{HONG2016914}.
Literature provides a wide range of electricity demand forecasting methods, with probabilistic approaches being favoured due to non-linear and non-stationary demand profiles \citep{mocanu2016demand}.
...
On Section \ref{sss:eval} we introduce the continuous ranked probability score (CRPS), the evaluation criteria of choice for the produced density forecasts. Finally, on Section \ref{sss:gof.min} details the implementation and results for the dataset used on this paper.
...
$\alpha_i$, $\beta_i$, $\delta_i$ and $\gamma_i$ are the coefficients of the AR, MA, GARCH and ARCH components with orders defined by non-negative integers $p$, $q$, $r$ and $m$, respectively;
...
For the distribution selection, \texttt{\textbf{rugarch}} enables the use of \textit{normal}, \textit{student-t}, \textit{generalised error distribution} and their skewed versions.
Given that \citet{theodossiou2015skewed} investigated empirical distributions such as the ones available in \texttt{\textbf{rugarch}} for finance data - with the presence of skewness, kurtosis, conditional heteroskedasticity, asymmetric volatility, and other non-linear dependencies.
These characteristics were responsible for biased models being trained when applying normal distributions.
The \textit{skewed version of the generalised error distribution} was proven to provide better fit to the empirical data distribution and, based on this - and to accommodate a highly volatile dataset - we chose it for the ARMA-GARCH model.
...
Similar to the unconditional KDE implementation done by \citet{Arora201647}, \citet{Jeon2016991} and \citet{Taylor2015370}, the method used on this work enables the non-parametric estimation of a probability density \(f\) based on observations \(\{Y_{1},Y_{2},...,Y_{n}\}\).
...
\begin{figure}
  \centering
  \includegraphics[scale=0.5]{2017-10-13_compare_fulldensities_KO2}
...
%-------------------------------------------
\section{Forecast-based Portfolio Optimisation}
\label{sec:portopt}
%-------------------------------------------
...
However, energy demand for different portfolio of customers does not behave like assets for a few reasons:
\begin{enumerate}
   \item The perishable nature of electricity demand;
   \item Affiliation restrictions, as customers either participate to a group or not and;
...
%-------------------------------------------
\subsection{Fit-for-purpose optimisation functions}
\label{ss:portopt}
%-------------------------------------------
...
\begin{enumerate}
   \item Forecast Validated (FV): minimising forecast error using the forecasting function on an (unseen) validation dataset.
   \item Seasonal Residual (SR): minimising the standard deviation of the residual of a deseasonalised aggregated demand and;
...
Considering the affiliation restrictions, $v$ was defined as a binary vector with as many positions as the number of households (equation \ref{eq:nonlfx}).
\begin{equation}
...
Using derivatives is not mandatory, but in some cases can speed up the convergence on local optima.
...
The average of these evaluations was returned as the FV function result, and as described in Section \ref{ss:portopt}, $v$ was chosen in order to minimise this result.
...
Finally it calculates the standard deviation of this single time-series data.

In other words, the method applies the following $F_{sr}$ function onto the undivided matrix $\bm{A}$ and the selection vector $v$, as shown in equation \ref{eq:sr}.
...
$R^{\bm{A} \times v}_t$ is defined as the remainder of the seasonal decomposition (described on Section \ref{ss:dtransf}) from the time-series generated after multiplying matrix $\bm{A}$ with the selection vector $v$, as in equation \ref{eq:stlsr}.
...
The first part of Equation \ref{eq:ss} calculates the similarity between the seasonal components of selected customers.
Lower $\widehat{RSD}(\bm{S^B_t})$ per column means minimal difference between seasonal signals per time-step.
The second part of Equation \ref{eq:ss} calculates the similarity between the remainder components of selected customers.
Since it is expected that $\bm{R^B_t}$ to be around zero, a second consequence is that lower $\widehat{RSD}(\bm{R^B_t})$ means a group of customers with lower variability at the same time, after removing the seasonal signal.
Different ratios $r$ can be used to change the weights between the objective functions.
...
For the same dataset, giving a weight  $r = 1.0$ to the seasonal component (which means ignoring the remainder similarity on the bi-objective function), returned the best accuracy.
...
\citet{CHANG20001271} describes the genetic algorithm (GA) as an “intelligent” probabilistic search algorithm, originally based on biological evolutionary process seen in nature.
Populations evolve following natural selection, where fitter individuals - and their genes - have greater chances of survival and reproduction.
Highly fit genes are more likely to be combined into offsprings which could, potentially, be even more fit.
Theoretically, species evolve as their populations become better adapted.

The GA computationally replicates the process by starting off with a population of individuals (initial guesses in the solution space), and applying reproduction and mutation operations.
The fitness of individuals is evaluated with respect to the optimisation objective function, and better fit individuals are given more reproduction chances by combining with other fit individuals.
After reproduction is taken care of, the mutation process includes random variability into offspring.
The process is repeated until a stoping condition is reached.

In this work, the genetic algorithm was set to stop after $100$ generations or after $10$ consecutive generations without enhancement in the objective function, whatever happened first.
...
Instead of running the optimisation to find one unique optimal group, we evenly divided the expected demand axis into $16$ ranges, defining $16$ optimisation scopes.
This method was chosen to enable the construction of a stepwise efficiency frontier for each optimisation scope.
Practically this means that, for any chosen function, instead of running a single optimisation run on $\bm{A}$ for the full spectrum of expected demand, we run $16$ optimisation scopes with different and non-super-positioned expected demand limits.
...
%===========================================
Figures \ref{fig:optKO_04h} and \ref{fig:optKO_12h} present the portfolio optimisation for two different time-ahead forecasts (4 and 12 hours ahead, respectively). This results are the average values from 11 different simulations, each containing 100 customers from the Korean dataset.
...
As the mean demand increases, a simple random aggregation of customers is able to reduce most of the uncertainty, leaving limited space for optimisation (upper part of the figures).
Nevertheless the portfolio optimisation returns the best available groups (or very close to it) most of the time, specially for the SR and FV functions.
On both forecast lead-times the SS function perform poorly compared to the alternatives.
Also, one can visually inspect that a longer forecast lead-time render a worse accuracy for random groups of customers, which does not hold for the optimally selected groups.
...
The FV function needs thousands of forecast runs while grouping, and this can get very resource intensive.
